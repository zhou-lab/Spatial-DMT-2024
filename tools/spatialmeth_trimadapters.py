...
with open_fastq_file(args.read1_fastq) as r1h, open_fastq_file(args.read2_fastq) as r2h:
    for rec1, rec2 in zip(SeqIO.parse(r1h, "fastq"), SeqIO.parse(r2h, "fastq")):
        ...
        if args.protocol == "smcseq":
            ...
            umi_seq = str(r2seq[umi_start:genomic_start])
            new_seq2 = r2seq[genomic_start:]
            new_qual2 = rec2.letter_annotations["phred_quality"][genomic_start:]
        else:
            ...
            ## lambda write precedes demux fail (kept for spatialdmt parity)
            ...
            umi_seq = "N"  # spatialdmt has no UMI; placeholder so biscuit -9 parses correctly
        ...
        rid1 = f"{rec1.id}_{cb_tag}_{umi_seq}"
        rid2 = f"{rec2.id}_{cb_tag}_{umi_seq}"
